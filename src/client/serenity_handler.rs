...
impl EventHandler for SerenityHandler {
    async fn cache_ready(&self, ctx: Context, _guilds: Vec<GuildId>) {
        ...
        let pi_hole_last_char: char = match pi_hole_url.chars().last() {
            None => {
                log::error!("An error occurred while parsing the Pi-Hole URL. You must enter a valid URL.");
                std::process::exit(1);
            }
            Some(last_char) => last_char
        };
        pi_hole_url = if pi_hole_last_char.to_string() == "/" { pi_hole_url } else { pi_hole_url + "/" };
        ...
        let channel_id: u64 = match std::env::var("CHANNEL_ID") {
            ...
            Err(error) => {
                log::error!("An error occurred while converting the channel ID: {}", error);
                std::process::exit(1);
            }
        };
